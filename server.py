...
def check_rate_limit(token: str) -> bool:
    ...
    now = time.time()
    # Clean old entries
    rate_limit_storage[token] = [t for t in rate_limit_storage[token] if now - t < RATE_LIMIT_WINDOW]
    # Check limit
...
if '*' in allowed_origins:
    cors_origins = ["*"]
    print("⚠️  WARNING: CORS configured for ALL origins (*) - this may be insecure for production!")
else:
...
async def webhook(request: Request, x_auth_token: str = Header(None)):
    ...
    if x_auth_token not in TOKEN_CONFIG:
        logger.warning(f"Unauthorized webhook attempt from {client_ip} with token: {redact_token(x_auth_token or 'none')}")
    ...
    if not check_rate_limit(x_auth_token):
        logger.warning(f"Rate limit exceeded for token {redact_token(x_auth_token)} from {client_ip}")
        return JSONResponse(content={"error": "Rate limit exceeded"}, status_code=status.HTTP_429_TOO_MANY_REQUESTS)

    # Check payload size
    content_length = request.headers.get("content-length")
    if content_length and int(content_length) > MAX_PAYLOAD_SIZE:
        logger.warning(f"Payload too large from {client_ip}: {content_length} bytes")
        return JSONResponse(content={"error": "Payload too large"}, status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE)

    channel = TOKEN_CONFIG[x_auth_token]['channel']
    try:
        data = await request.json()
        logger.info(f"Webhook received from {client_ip} for channel '{channel}' (token: {redact_token(x_auth_token)})")
...
async def ws_channel_endpoint(websocket: WebSocket, channel: str, token: Optional[str] = None):
    ...
    if origin and not any(origin.startswith(f"https://{domain}") or origin.startswith(f"http://{domain}")
                        for config in TOKEN_CONFIG.values()
                        for domain in [config['domain']] if config['domain'] != '*'):
        # If any token has domain restrictions, validate origin
        has_domain_restrictions = any(config['domain'] != '*' for config in TOKEN_CONFIG.values())
        if has_domain_restrictions:
            logger.warning(f"WebSocket origin validation failed from {client_ip} (origin: {origin})")
    ...
    if token not in TOKEN_CONFIG or TOKEN_CONFIG[token]['channel'] != channel:
        logger.warning(f"WebSocket auth failed from {client_ip} for channel '{channel}' (token: {redact_token(token or 'none')})")
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
        return

    logger.info(f"WebSocket connected from {client_ip} to channel '{channel}' (token: {redact_token(token)})")
    
    # Ensure the channel queue exists
    if channel not in message_queues:
        logger.error(f"Channel '{channel}' does not exist for WebSocket connection from {client_ip}")
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
        return
...
async def ws_endpoint(websocket: WebSocket, token: Optional[str] = None):
    ...
    if token not in TOKEN_CONFIG:
        logger.warning(f"WebSocket auth failed from {client_ip} (token: {redact_token(token or 'none')})")
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
        return

    # Route to the token's assigned channel
    channel = TOKEN_CONFIG[token]['channel']
    logger.info(f"WebSocket connected from {client_ip} to channel '{channel}' (legacy endpoint, token: {redact_token(token)})")
    ...
    try:
        ...
        logger.info(f"WebSocket disconnected from {client_ip} for channel '{channel}' (legacy endpoint)")
        await websocket.close()
